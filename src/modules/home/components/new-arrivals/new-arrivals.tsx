...
const NewArrivals = ({ countryCode, initialProducts }: NewArrivalsProps) => {
  ...
  const hasDragInteracted = useRef(false)
  ...
  const handleMouseUp = useCallback(() => {
    ...
    if (dragStartRef.current) {
      ...
      // Track that user has interacted via drag (disable scroll-jacking)
      if (lastDragDistanceRef.current > 10) {
        hasDragInteracted.current = true
      }
    }
  ...
  // Page bottom detection (desktop only)
  useEffect(() => {
    // Only on desktop (lg+)
    ...
    const handleScroll = () => {
      ...
      // User is at or past the bottom (with small threshold)
      const atBottom = scrollTop + clientHeight >= scrollHeight - 10
      setIsAtPageBottom(atBottom)
    }
  ...
  // Wheel event listener for scroll-jacking (desktop only)
  useEffect(() => {
    const checkIfDesktop = () => window.innerWidth >= 1024
    ...
    const handleWheel = (e: WheelEvent) => {
      ...
      // Don't hijack if user has dragged manually
      if (hasDragInteracted.current) return

      // Don't hijack if currently dragging
      if (isDragging) return

      // Only hijack when at page bottom
      if (!isAtPageBottom) return

      const currentScrollOffset = scrollOffset
      const canScrollRight = currentScrollOffset < maxScroll
      const canScrollLeft = currentScrollOffset > 0

      // Scrolling down (deltaY > 0) - try to scroll products right
      if (e.deltaY > 0 && canScrollRight) {
        e.preventDefault()
        const newOffset = Math.min(currentScrollOffset + e.deltaY * 2, maxScroll)
        setScrollOffset(newOffset)
        velocityRef.current = 0 // Stop any momentum
        return
      }

      // Scrolling up (deltaY < 0) - try to scroll products left
      if (e.deltaY < 0 && canScrollLeft) {
        e.preventDefault()
        const newOffset = Math.max(currentScrollOffset + e.deltaY * 2, 0)
        setScrollOffset(newOffset)
        velocityRef.current = 0 // Stop any momentum
        return
      }

      // If we can't scroll horizontally anymore, allow normal vertical scroll
      // (no preventDefault, browser handles it naturally)
    }
  ...
  }, [isAtPageBottom, isDragging, scrollOffset, maxScroll])
