...
const CustomPaymentSelector = <TMethod extends string>({
  ...
  return (
    <RadioGroup value={selectedMethod} onChange={onMethodSelect} className="space-y-0">
      {availableMethods.map((method) => {
        const detail = selectedMethod === method.id ? renderPaymentDetails(method.id) : null
        ...
        return (
          <div key={method.id} className="flex flex-col">
                ...
                {method.icons.map((IconComponent, index) => (
                  <div
                    ...
                    className={clx(
                      ...
                      selectedMethod === method.id ? "opacity-100" : "opacity-50 grayscale"
                    )}
                  >
                    <IconComponent />
                  </div>
            ...
            {selectedMethod === method.id && hasDetail && (
              <div className="ml-8 animate-fadeIn">{detail}</div>
            )}
          </div>
        )
      })}
