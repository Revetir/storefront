  ...
  if (parts.length < 2) {
    ...
    const product = await listProducts({
      ...
      queryParams: {
        ...
        fields: "handle,title,description,thumbnail,*images,*categories,+product_sku.*,+brand.*,*variants.calculated_price,+variants.inventory_quantity,+variants.manage_inventory,+variants.allow_backorder,*options.*",
        limit: 1,
      },
...
export default async function ProductPage(props: Props) {
  ...
  const pricedProduct = await listProducts({
    ...
    queryParams: {
      ...
      fields:
        "*images,*variants.calculated_price,+variants.inventory_quantity,+variants.manage_inventory,+variants.allow_backorder,+metadata,+tags,*categories,+product_sku.*,+brand.*",
    },
  }).then(({ response }) => response.products[0])
