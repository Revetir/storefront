...
export default async function ProductPage(props: Props) {
  ...
  const pricedProduct = await listProducts({
    ...
    queryParams: {
      ...
      fields:
        "*images,*variants.calculated_price,+variants.inventory_quantity,+variants.manage_inventory,+variants.allow_backorder,+variants.options.*,*variants.metadata,+variants.ean,+variants.upc,+variants.barcode,+metadata,+tags,*categories,+product_sku.*,+brand.*,*options.*",
    },
  }).then(({ response }) => response.products[0])
