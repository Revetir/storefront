...
export async function retrieveCart(cartId?: string) {
  ...
  return await sdk.client
    .fetch<HttpTypes.StoreCartResponse>(`/store/carts/${id}`, {
      ...
      query: {
        fields:
          "*items, *region, *items.product, *items.variant, *items.variant.calculated_price.*, *items.variant.pricing.*, *items.thumbnail, *items.metadata, +items.total, +items.original_total, *promotions, +shipping_methods.name, +items.product.brands.*",
      },
      headers,
      next,
      cache: "force-cache",
