...
export function generateProductJsonLd({ product, region, countryCode }: ProductJsonLdProps): string {
  ...
  const variantEntries = product.variants?.map((variant, index) => {
    ...
    // Debug: Log the variant structure
    console.log('Variant in JSON-LD:', JSON.stringify(variant, null, 2))
    
    // Get variant name (size) - check if options array has valid objects
    const variantName = product.options?.[0]?.values?.[index]?.value || variant.title || `variant-${index}`
